			...
			for (int i = 0; i < instrutores.size(); i++) {
				if (instrutores.get(i).getCurso().getId() == id) {
					...
					"\nId: "+ instrutores.get(i).getCurso().getId() +
					 "\nData de In�cio: " + instrutores.get(i).getCurso().getDatainicio() +
					  "\nData de T�rmino: " + instrutores.get(i).getCurso().getDatatermino());
					break;
				}
			}
	...
	public void atualizarCursos(int id, String nomeCurso, String datainicio, String datatermino) {
		...
		for (int i = 0; i < instrutores.size(); i++) {
			if (instrutores.get(i).getCurso().getId() == id) {
				...
				System.out.println("Curso n�o encontrado, tente outro!");
			}
		}
	}
